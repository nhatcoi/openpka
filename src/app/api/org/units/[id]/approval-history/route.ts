...
// GET /api/org/units/[id]/approval-history
export const GET = withErrorHandling(async (_request: Request, ctx?: { params?: Promise<{ id: string }> }) => {
  ...
  const orgUnitId = BigInt(id);

  const workflow = await engine.getWorkflowByEntity('ORG_UNIT', orgUnitId);
